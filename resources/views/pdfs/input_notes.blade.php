...
<head>
	<meta charset="UTF-8">
	<title>Nota de Ingreso: {{ $model->sn }}-{{ $model->created_at->formatLocalized('%Y') }}</title>
	<link rel="stylesheet" href="./css/order_pdf.css">
</head>
<body>
	<div class="header">
		<div class="title">NOTA DE INGRESO: {{ str_pad($model->sn, 3, '0', STR_PAD_LEFT) }} - {{ $model->created_at->formatLocalized('%Y') }}</div>
